...
param (
   ...
   [Parameter(Mandatory=$true)]
   [string]$BillingProfileId,  # Example = "F4AY-XXXX-...",
   [Parameter(Mandatory=$true)]
...
while ($currentDate -le $endDate) {
   ...
   $apiUrl = "https://management.azure.com/providers/Microsoft.Billing/billingAccounts/$($BillingAccountId)/billingProfiles/$($BillingProfileId)/providers/Microsoft.Consumption/reservationTransactions?api-version=2024-08-01&grain=daily&`$filter=properties/EventDate+ge+$($monthStart.ToString("yyyy-MM-dd"))+AND+properties/EventDate+le+$($monthEnd.ToString("yyyy-MM-dd"))"
   if($null -eq $results) {
